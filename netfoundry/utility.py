...
class Utility:
    ...
    def get_token_cache(self, path):
        ...
        try:
            ...
            logging.debug("parsed token cache file '%s' as JSON with mode %s", path.__str__(), cache_file_stats.filemode(cache_file_stats.st_mode))
    ...
    def jwt_audience(self, token):
        """Return an audience URL for Gateway Service.
